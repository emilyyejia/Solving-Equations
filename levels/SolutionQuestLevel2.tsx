...
const SolutionQuestLevel2: React.FC<LevelComponentProps> = ({ onComplete, onExit, partialProgress, onSavePartialProgress }) => {
  ...
  const handleP1 = () => {
    ...
    if (p1Phase === 'eq') {
      ...
      else { showFeedback("Not quite! Look at the word problem carefully. What's the pen cost plus the notebook cost?", 'error'); }
    } else {
      ...
      else { setP1Errors(newErrors); showFeedback("Almost there! Combine like terms first. What do you get when you add the X terms together?", 'error'); setTimeout(() => setP1Errors(new Set()), 2000); }
    }
  };
  ...
  const handleP2 = () => {
    if (p2Phase === 'eq') {
      ...
      else { showFeedback("Not quite! Take another look at how you built the equation.", 'error'); }
    } else {
      if (p2Students === '3') { showFeedback("Correct!", 'success'); setTimeout(() => { setStep(3); setFeedback(null); }, 1000); }
      else { showFeedback("Almost there! Simplify the equation first, then solve for X.", 'error'); }
    }
  };
  ...
  const handleP3 = () => {
    ...
    if (p3Phase === 'eq') {
      ...
      else { showFeedback("Not quite! Break the sentence into parts: What does '3 times a number' mean? What about 'decreased by 4'?", 'error'); }
    } else {
      if (p3Value === '9') { setFeedback(null); setIsLevelComplete(true); }
      else { showFeedback("Hmm… the mystery isn't solved yet. Does your value make both sides equal?", 'error'); }
    }
  };
          ...
          <div className="animate-fade-in flex flex-col items-center">
             ...
             {p1Phase === 'eq' ? (
                <div className="w-full space-y-6">
                   <p className="text-center text-gray-300 text-xl font-bold">Pick the correct equation (x = pen cost):</p>
        ...
        {step === 2 && (
          <div className="animate-fade-in flex flex-col items-center">
             <div className="bg-gray-900/50 p-8 rounded-2xl mb-10 border-l-4 border-indigo-500">
                <p className="text-2xl leading-relaxed italic text-white">"Three adult tickets cost $12 each, and student tickets cost $6 each. The total cost was <span className="text-emerald-400">$54</span>. How many student tickets were bought?"</p>
             </div>
             {p2Phase === 'eq' ? (
                <div className="w-full space-y-6">
                   <p className="text-center text-xl font-bold text-gray-300">Choose the matching equation (x = student count):</p>
                   <div className="grid grid-cols-1 gap-4">
          ...
          <div className="animate-fade-in flex flex-col items-center">
             ...
             {p3Phase === 'eq' ? (
                <div className="w-full space-y-6 max-w-md">
                   <p className="text-center text-gray-300 font-bold text-xl">Write the full equation:</p>
                   <input type="text" value={p3EqInput} onChange={e => setP3EqInput(e.target.value)} className="w-full bg-gray-900 border-4 border-sky-500 p-4 rounded-xl text-center text-2xl font-mono text-white" placeholder="e.g. 2x + 1 = ..." />
                ...
                <div className="animate-fade-in-up space-y-6 w-full max-w-md flex flex-col items-center">
                   <p className="text-center text-gray-300 font-bold text-2xl">Solve the equation and find the value of X.</p>
                   <p className="text-center font-mono text-4xl text-white">3X - 4 = 2X + 5</p>
