...
import type { Landmark, DraggableItem, PlacedBlock, BlockOrientation, LevelSolution } from '../../levels/custom-map-scales-2/mapScalesTypes2';
...
interface GridMapProps {
  ...
  onInvalidDrop: (reason: 'scale' | 'location') => void;
  ...
  const [{ isOver }, drop] = useDrop(
    () => ({
      ...
      drop: (item: DraggableItem, monitor) => {
        ...
        // If the scale is wrong, it's an invalid scale drop.
        if (item.size !== scale) {
          onInvalidDrop('scale');
          return;
        }
        ...
        // Only add block if placement is correct.
        if (isCorrect) {
          onAddBlock({
              position: { row: snappedRow, col: snappedCol },
              orientation: item.orientation,
              isCorrect: true, // Always true now
              size: item.size,
          });
        } else {
          // Since we already checked the scale, any other failure is location-based.
          onInvalidDrop('location');
        }
      },
      collect: (monitor) => ({
    ...
    [onAddBlock, onInvalidDrop, checkPlacement, rows, cols, scale, level, solution]
  );
    ...
    <div className="w-full">
        ...
        <div className="grid grid-cols-[auto_1fr] grid-rows-[1fr_auto] w-full" style={{ aspectRatio: `${cols}/${rows}` }}>
            ...
            <div ref={gridRef} style={mainGridStyle}>
                ...
                {!isCompleted && placedBlocks.map((block) => {
                    const bgColor = 'bg-green-500/80';
                    ...
                    if (block.orientation === 'horizontal') {
                        const blockWidthPercent = (block.size / scale) * cellWidthPercent;
                        ...
                        const blockHeightPercent = (block.size / scale) * cellHeightPercent;
